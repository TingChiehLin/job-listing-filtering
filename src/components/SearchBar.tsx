...
const SearchBar: FC<SearchBarProps> = ({ tags, onAddTag, onRemoveTag, onClearTags }) => {
  const [searchText, setSearchText] = useState<string>("");

  return (
    <div className="relative">
      <ul
        className="absolute top-[62px] left-[11.5%] -mt-10 cursor-pointer"
      >
        {searchText.length > 0 ? (
          <AiOutlineClose size={"1.5rem"} onClick={() =>{ setSearchText('') }} />
        ) : (
